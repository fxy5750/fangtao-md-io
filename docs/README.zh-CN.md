...
- 多文件导入时由浏览器逐个上传并处理，避免一次请求占用过多资源，并展示每个文件的进度与结果。
...
- 可选从 JPEG 导入临时副本中移除 XMP APP1 元数据，同时保留 EXIF 与源 ZIP 文件。
...
1. 可一次选择多个支持的 Markdown 文本文件或 `.zip` 压缩包。
2. 选择导入到的内容类型。
3. 导入普通文章时，可选择要归入的分类。
4. 选择 **草稿** 或 **立即发布**。日期留空时会使用 ZIP 内每篇 Markdown 的最后修改时间；也可手动设置精确到秒的发布日期，并可选择私密可见性或访问密码。
5. 选择 Markdown 解析器。
6. 点击 **上传并导入**。

不包含任何受支持 Markdown 文件的 ZIP 会被自动跳过，压缩包内的素材不会导入媒体库。

日期优先级依次为：手动发布日期、ZIP 内 Markdown 的最后修改时间、Front Matter 的 `date`、WordPress 当前时间。

批量上传多个 ZIP 且不填写发布日期时，插件会分别读取每个压缩包内每篇 Markdown 的最后修改时间；不同压缩包和不同 Markdown 可以生成不同的文章发布时间。

每个 Markdown 文件会创建一篇新的 WordPress 内容。导入页面支持文章、页面，以及当前用户有权编辑的公开自定义文章类型。

私密可见性与访问密码是 WordPress 的两种独立访问方式，导入表单中一次只能选择其中一种。

管理员可在页面底部的 **导入设置** 中保存默认文章状态和解析器、选择 ZIP 素材格式、设置导入大小限制，并决定是否导入远程图片或忽略 JPEG XMP 元数据。大小填写 `0` 或留空时跟随当前 PHP/WordPress 上传限制；远程图片导入与 XMP 清理默认关闭。XMP 清理只处理导入过程的 JPEG 临时副本，不会修改 EXIF 或源 ZIP 文件。

导入失败后，请先在当前插件版本中重现一次，再打开 **高级功能 > 下载诊断日志**。仅管理员可以下载该 JSON 文件，其中包含服务器能力、实际导入限制、启用插件版本和最近导入结果；不会包含密码、凭据、文章正文、站点地址或服务器绝对路径。分享前仍建议检查文件名和已清理的错误摘要。
...
- 新增可选的 JPEG XMP 兼容设置，用于未启用 PHP DOM 扩展的服务器。

### 1.9.19

- 在“高级功能”中新增仅管理员可用的 JSON 诊断日志导出，并对路径、网址等信息脱敏；后续导入日志会记录更完整的失败环境与请求上下文。

### 1.9.18

- 在继续启用 Markdown 解析器安全模式的前提下，安全保留并清理完整的 HTML 表格。
- 当正文引用的图片无法导入媒体库时，停止并回滚当前内容，避免发布包含失效相对图片地址的文章。
...
- 统一发布日期窗格中“当前时间”和“清空”操作图标的视觉居中对齐。

### 1.9.14

- 修复发布日期窗格脱离输入框的问题；日期与时间合并为一个可精确到秒的统一控件。

### 1.9.13

- 修复发布日期弹窗的打开状态，并在空间不足时自动向上展开；统一“当前时间”和“清空”按钮图标对齐。

### 1.9.12

- 将文章发布日期改为紧凑输入框：点击后弹出日期时间窗格，支持手动输入或选择日期时间，并提供“当前时间”和“清空”快捷操作；留空时仍自动读取 ZIP 内 Markdown 的修改时间。

### 1.9.11

- Markdown 导入与导出页面改为使用 WordPress 后台可用内容宽度，并继续在窄屏下自适应折叠。

### 1.9.10

- 导入统计改为读取持久化日志，刷新页面后仍会保留汇总数据与最近导入记录。
- 新增“高级功能”：二次确认后仅清除导入统计与日志，不删除文章、媒体或导入设置。

### 1.9.9

- Markdown 与 ZIP 文件选择框新增清空所选文件按钮。

### 1.9.8

- 优化导入环形图：增加百分比引线标签、横向图例，并支持鼠标悬停或键盘聚焦时强调对应数据扇区。

### 1.9.7

- 将导入统计条改为实时环形图，按成功、失败和无效内容显示比例，并在圆心显示已处理总数。

### 1.9.6

- 恢复插件下拉框右侧箭头，并为亮色和深色后台提供匹配的可见颜色。

### 1.9.5

- 优化 Markdown 导入与导出页面的后台界面：两页统一使用完整工作区宽度，表单层级、控件状态和内容入口卡片更清晰，并适配窄屏后台。

### 1.9.4

- 右侧导入区域改为常驻统计仪表板和任务队列：统计条按成功、失败、无效内容的数量变化；每个任务显示名称、状态和进度。

### 1.9.3

- 导入队列常驻显示；扩大桌面端导入区域，并在较窄的后台宽度下自动改为上下布局，避免压缩导入表单。

### 1.9.2

- 多文件导入改为浏览器逐个上传和处理，新增每个文件的进行中、成功、跳过和失败反馈。

### 1.9.1

- 修复导入图片附件标题为空时，媒体库被错误显示为“上传中”的问题。
- 未手动设置发布日期时，使用 ZIP 内每篇 Markdown 的最后修改时间。
